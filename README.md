...
* **Line 10:** `ee.ria.DigiDoc.idcard.SignatureAlgorithmException` – thrown by `signatureAlgorithm(...)` when the library cannot sign with that certificate's key, and by the signing calls when the hash does not suit the algorithm the card named.
  ...
  It is raised before the PIN is verified in the first two cases, so those cost no PIN retry — the digest is checked while the input is prepared, which happens before the VERIFY. For Latvian cards the algorithm and key references come from the card's own PKCS#15 metadata; Estonian cards use constants.
